...
// Bound to 127.0.0.1, but loopback is shared by everything running on the
// machine: any page open in the same browser can already reach this port.
// Without these checks, a page from an unrelated origin could POST to
// /submit, /autosave, or /font-upload blind and have this process write into
// the project and spawn a document session on its behalf (the same shape of
// hole fix/555 closed on serve-question.mjs). Host and Origin are checked
// unconditionally, on every request, because this server has no equivalent
// of that fix's detached-mode session key: it always runs attended, so origin
