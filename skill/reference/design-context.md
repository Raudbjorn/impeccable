...
Loaded by `{{command_prefix}}impeccable design-context`. Owns the design interview record and its portable form (export/import). The interview itself is created by `{{command_prefix}}impeccable document` seed mode; this command is everything afterwards.
