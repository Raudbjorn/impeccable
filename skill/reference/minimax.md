...
```sh
IMAGE_GEN_PROVIDER=minimax node "{{scripts_path}}/image-gen.mjs" --prompt "A close, wordless botanical still life for the florist landing page, pale linen, deep green leaves and one vivid coral bloom, crisp focus and even daylight." --out "/absolute/path/hero.png"
```
...
```sh
node "{{scripts_path}}/image-analyze.mjs" --image "screenshot.png" --mode quick
node "{{scripts_path}}/image-analyze.mjs" --image "screenshot.png" --mode detailed --prompt "For the request to tighten this header, identify the gap marked by the arrow and the elements on either side. Report visible evidence and any ambiguity."
```
...
```sh
node "{{scripts_path}}/image-analyze.mjs" --image "screenshot.png" --no-cache
node "{{scripts_path}}/image-analyze.mjs" --clear-cache
```
...
```sh
node "{{scripts_path}}/web-search.mjs" --query "MiniMax M3 image input API documentation" --limit 5
```
