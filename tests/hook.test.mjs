...
describe('hook-admin.mjs', () => {
  ...
  // oh-my-pi's hook is a whole JS module, not a JSON manifest. Everything
  // below exercises the module-shaped target through the same reset contract
  // the JSON targets use.

  const OMP_MODULE_REL = path.join('.omp', 'hooks', 'post', 'impeccable.js');
  const ompModule = (extra = '') =>
    `// @impeccable-hook-module v1 - generated by impeccable\n`
    + `export default function impeccableHook(pi) {}\n// hook.mjs\n${extra}`;

  function writeOmpModule(contents) {
    const dest = path.join(cwd, OMP_MODULE_REL);
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    fs.writeFileSync(dest, contents);
    return dest;
  }

  it('reset removes the oh-my-pi hook module and names the provider', () => {
    const dest = writeOmpModule(ompModule());

    const out = runAdmin(['reset']);

    assert.match(out, /Removed hook entries from: \.omp\./);
    assert.equal(fs.existsSync(dest), false, 'the module is deleted, not pruned');
  });

  it('reset removes a module written before the marker existed', () => {
    // Bundles shipped an unmarked module. Refusing it would let reset report
    // success while the hook stayed armed (the issue #512 class).
    const dest = writeOmpModule('export default function impeccableHook(pi) {}\n// runs hook.mjs\n');
  ...
  it('reset leaves a foreign module at our path byte-for-byte unchanged', () => {
    const foreign = 'export default function somebodyElse(pi) { pi.on("tool_result", () => {}); }\n';
    const dest = writeOmpModule(foreign);

    const out = runAdmin(['reset']);

    assert.doesNotMatch(out, /Removed hook entries from/);
    assert.equal(fs.readFileSync(dest, 'utf-8'), foreign);
  });

  it('a module naming the hook script path still resets instead of deadlocking', () => {
    // The JSON "wired but unparseable" check scans raw text for command
    // markers and then requires JSON.parse to fail. A module never parses as
    // JSON, so routing it through that check would make every reset throw.
    const dest = writeOmpModule(ompModule('const p = "skills/impeccable/scripts/hook.mjs";\n'));
  ...
  it('reset refuses to remove the config when the module cannot be deleted', (t) => {
    if (process.getuid?.() === 0) {
      t.skip('a read-only directory does not stop root from unlinking');
      return;
    }
    fs.mkdirSync(path.join(cwd, '.impeccable'), { recursive: true });
    fs.writeFileSync(getConfigPath(cwd), JSON.stringify({ hook: { enabled: false } }));
    const dest = writeOmpModule(ompModule());
    // rmSync on a read-only *file* succeeds on POSIX; the directory is what
    // has to be locked to make the unlink fail.
    ...
    assert.ok(err, 'an undeletable module must fail the reset');
    assert.equal(err.status, 1);
    assert.match(err.stderr, /\.omp\/hooks\/post\/impeccable\.js/);
  ...
  it('status reports the oh-my-pi module as wired', () => {
    writeOmpModule(ompModule());
    assert.match(runAdmin(['status']), /wiring:\s+\.omp/);
  });

  it('on confirms an installed module and reports a missing one', () => {
    fs.mkdirSync(path.join(cwd, '.omp', 'skills', 'impeccable'), { recursive: true });

    // The module's content is a build artifact hook-admin does not carry, so
    // `on` can confirm or report it, never author it.
    const missing = runAdmin(['on']);
    assert.match(missing, /No hook file installed for: \.omp\./);
    assert.match(missing, /skills install/);

    writeOmpModule(ompModule());
    const present = runAdmin(['on']);
    assert.match(present, /already installed for: .*\.omp/);
    assert.doesNotMatch(present, /No hook file installed/);
  });
