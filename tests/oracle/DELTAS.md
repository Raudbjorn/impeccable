...
## Recorded 2026-09-25: fake plate mode stops resampling the crop it is scored against

`gate_one_plate`'s crop-identity check (`is_transformed_copy` / structural
similarity against the raw region) used to exempt any file carrying an
`impeccable:fake` text chunk, so `--plate` in fake mode passed by resampling
the reference crop itself; `caller_supplied_fake_metadata_cannot_bypass_the_crop_check`
(Rust, `crates/comp-verbs/src/build_phase/integrity_tests.rs`) now pins that a
caller-supplied tag must never bypass that check, since nothing stops a real
caller from writing the same tag on a real plate. Fake mode now tints the
resized crop with a coarse checkerboard offset instead of resampling it bare:
enough to move every pixel well past the crop check's copy tolerance while
keeping the region's gross composition recognizable to the fidelity score.

- `genimg-plate-fake`, `genimg-plate-score-only`, `genimg-plate-score-only-min-rejects`: stdout's `fake 2x crop of region` becomes `fake tinted crop of region`, the plate PNG bytes change, and the reported score moves from 78% (structure 99%, color 78%, detail 65%) to 74% (structure 93%, color 70%, detail 64%) — still comfortably above every gate threshold. Exit codes, prompts, sidecar JSON and every other field are unchanged.
