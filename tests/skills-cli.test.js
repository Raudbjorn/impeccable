...
describe('copyProviderHooks: the oh-my-pi hook module', () => {
  function ompBundle(root) {
    // The real generator output, not a hand-copy of its shape. The installer
    // resolves the user-scope path by matching against this text, so a fixture
    // that reproduced the format by hand would keep passing after the
    // generator drifted and the installer stopped matching it.
    const bundleDir = join(root, 'bundle');
    const dir = join(bundleDir, '.omp', 'hooks', 'post');
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, 'impeccable.js'), buildOmpHookModule());
    return bundleDir;
  }

  test('installs the module for a project scope and keeps the self-relative path', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-omp-proj-'));
    const bundleDir = ompBundle(tmp);

    expect(copyProviderHooks(bundleDir, tmp, ['.omp'], { skillRoot: tmp })).toEqual(['.omp']);

    const written = readFileSync(join(tmp, '.omp', 'hooks', 'post', 'impeccable.js'), 'utf8');
    expect(written).toContain('import.meta.url');
    expect(written).not.toContain('const HOOK_SCRIPT = "/');

    // Second run is a no-op: the installer must not rewrite an identical file.
    expect(copyProviderHooks(bundleDir, tmp, ['.omp'], { skillRoot: tmp })).toEqual([]);

    rmSync(tmp, { recursive: true, force: true });
  });

  test('resolves the hook script into the home skills dir for a user scope', () => {
    // oh-my-pi reads user skills from ~/.omp/agent/skills while the hook file
    // still lands in the project, so the self-relative walk cannot reach it.
    const tmp = mkdtempSync(join(tmpdir(), 'imp-omp-user-'));
    const skillHome = mkdtempSync(join(tmpdir(), 'imp-omp-home-'));
    const bundleDir = ompBundle(tmp);

    copyProviderHooks(bundleDir, tmp, ['.omp'], { skillRoot: skillHome, scope: 'user' });

    const written = readFileSync(join(tmp, '.omp', 'hooks', 'post', 'impeccable.js'), 'utf8');
    expect(written).toContain(join(skillHome, '.omp', 'agent', 'skills', 'impeccable', 'scripts', 'hook.mjs'));
    expect(written).not.toContain(join(skillHome, '.omp', 'skills'));
    expect(written).not.toContain('import.meta.url');

    rmSync(tmp, { recursive: true, force: true });
    rmSync(skillHome, { recursive: true, force: true });
  });

  test('fails loudly when the generated module no longer matches the rewrite', () => {
    // The user-scope path is spliced in by matching generated text. A silent
    // no-op there would install a module whose self-relative walk points at a
    // skill that is not there, and still report success.
    const tmp = mkdtempSync(join(tmpdir(), 'imp-omp-drift-'));
    const bundleDir = ompBundle(tmp);
    const src = join(bundleDir, '.omp', 'hooks', 'post', 'impeccable.js');
    // Reorder the two imports: still valid JS, no longer the shape matched.
    writeFileSync(src, buildOmpHookModule()
      .replace('import { fileURLToPath } from "node:url";\nimport { dirname, join } from "node:path";',
               'import { dirname, join } from "node:path";\nimport { fileURLToPath } from "node:url";'));

    expect(() => copyProviderHooks(bundleDir, tmp, ['.omp'], { skillRoot: join(tmp, 'home'), scope: 'user' }))
      .toThrow(/no longer matches the shape this installer rewrites/);

    rmSync(tmp, { recursive: true, force: true });
  });

  test('refuses to clobber a hook file impeccable did not write, unless forced', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-omp-foreign-'));
    const bundleDir = ompBundle(tmp);
    const dest = join(tmp, '.omp', 'hooks', 'post', 'impeccable.js');
    mkdirSync(dirname(dest), { recursive: true });
    const foreign = 'export default function somebodyElse(pi) {}\n';
    writeFileSync(dest, foreign);

    expect(() => copyProviderHooks(bundleDir, tmp, ['.omp'], { skillRoot: tmp })).toThrow(/not written by impeccable/);
    expect(readFileSync(dest, 'utf8')).toBe(foreign);
