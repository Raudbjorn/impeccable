...
2. **Confirm the target slugs cleanly**:
   ```bash
   node .vibe/skills/impeccable/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
4. If mutation is available, start `node .vibe/skills/impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
4. **Read the trend** for context:
   ```bash
   node .vibe/skills/impeccable/scripts/critique-storage.mjs trend "<resolved target>" 5
   ```
