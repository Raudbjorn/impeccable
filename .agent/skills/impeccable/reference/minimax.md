...
```sh
IMAGE_GEN_PROVIDER=minimax node ".agent/skills/impeccable/scripts/image-gen.mjs" --prompt "A close, wordless botanical still life for the florist landing page, pale linen, deep green leaves and one vivid coral bloom, crisp focus and even daylight." --out "/absolute/path/hero.png"
```
...
```sh
node ".agent/skills/impeccable/scripts/image-analyze.mjs" --image "screenshot.png" --no-cache
node ".agent/skills/impeccable/scripts/image-analyze.mjs" --clear-cache
```
...
```sh
node ".agent/skills/impeccable/scripts/web-search.mjs" --query "MiniMax M3 image input API documentation" --limit 5
```
