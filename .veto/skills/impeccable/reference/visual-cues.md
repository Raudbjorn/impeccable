...
- **Unique**: `node .veto/skills/impeccable/scripts/visual-cues.mjs hash [paths]` prints one sha256 per file; each must be unique. Two identical heroes mean two subagents raced on a shared default output filename; re-spawn one of the pair and take its fresh file before compiling.
...
```text
node .veto/skills/impeccable/scripts/visual-cues.mjs compile [hero.png] \
  --slug [slug] \
  --palette "primary=#RRGGBB;secondary=#RRGGBB;tertiary=#RRGGBB;neutral=#RRGGBB" \
