...
Baseline: **605 findings**: 555 on `index.html`, 47 on the bundled stylesheet, and 3 on the bundled script (was 606: 556/47/3, until the <brand-name> placeholder-brand demo content was pruned; see `docs/add-branding.md`). The headline is dominated by per-element multiplication, not by distinct problems: the jsdom pass reports once per matching element, so one deliberate declaration can carry hundreds of hits, and a single one does: `picker.css`'s artboard morph, which eases `margin` and `padding` while the wireframe previews an answer, accounts for 527 of the `index.html` findings on its own (the morph is the preview mechanic; easing geometry on a miniature board is the point, not thrash). The distinct accepted groups below are the real ledger, each with a reason on record, so hold 605 rather than treating it as a backlog:
