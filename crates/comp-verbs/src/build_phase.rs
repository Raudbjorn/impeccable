...
fn gate_spec(io: &Io, state: &Value) -> Gate {
    ...
    let Some(spec) = load_spec(&abs(io, SPEC_PATH)) else {
        ...
        return Gate::fail(vec![format!(
            "no spec at {SPEC_PATH}: run impeccable comp-spec --comp {comp} --grid, name the regions, then --regions regions.json"
        )]);
    };
        ...
        return Gate::fail(
            cut.iter()
                .map(|r| {
                    ...
                    format!("region {id}: the comp's artwork runs off its box on the {sides}; widen the region's span so the box holds the whole shape with a margin (or set \"bleed\": true if the page really crops it there), then re-run impeccable comp-spec --regions")
                })
                .collect(),
        );
    }
...
fn plate_verdict(region: &Value, score: &Score) -> (bool, Vec<String>) {
    ...
    if is_texture {
        ...
        if effective < PLATE_MIN {
            reasons.push(format!(
                "scores {}% as the material of region {id} (color {}%, detail {}%); crop a clean patch of the comp region (impeccable comp-spec --crop {id} --raw) and mirror-tile it, generate only when no clean patch exists",
    ...
    if score.structure < PLATE_STRUCTURE_MIN {
        reasons.push(format!(
            "structure {}% against the comp region {id}: the composition of the plate is not the region's (different subject, orientation, or crop); regenerate with impeccable comp-spec --crop {id} as the reference image",
...
pub struct PlateGate {
    ...
    pub reference_audit: Option<Value>,
...
pub fn gate_one_plate(io: &Io, spec: &Value, comp: Option<&Image>, region: &Value, file_override: Option<&str>) -> PlateGate {
    ...
    let Some(file) = candidate.clone().filter(|f| abs(io, f).exists()) else {
        ...
        return PlateGate { reasons, score: None, status: "missing", file: candidate, size: None, asset_hash: None, region_hash: None, reference_hash: None, reference_audit: None, comp_hash: None };
    };
    let img = match std::fs::read(abs(io, &file)).map_err(|e| e.to_string()).and_then(|b| png_io::decode_png(&b)) {
        ...
        Err(e) => {
            ...
            return PlateGate { reasons, score: None, status: "unreadable", file: Some(file), size: None, asset_hash: None, region_hash: None, reference_hash: None, reference_audit: None, comp_hash: None };
        }
    };
    let is_texture = region.get("kind").and_then(Value::as_str) == Some("texture");
    ...
    let status = if reasons.is_empty() { "ok" } else { "invalid" };
    let asset_hash = sha256_file(io, &file);
    let region_hash = Some(sha256_bytes(util::json_pretty(region).as_bytes()));
    let reference_hash = Some(plate_reference_hash(spec));
    let comp_hash = spec.get("comp").and_then(Value::as_str).and_then(|p| sha256_file(io, p));
    PlateGate {
        reasons,
        score: score_val,
        status,
        file: Some(file),
        size: Some((img.image.width, img.image.height)),
        asset_hash,
        region_hash,
        reference_hash,
        reference_audit,
...
fn gate_plates_for(io: &Io, spec: &Value, only_id: Option<&str>) -> Gate {
    ...
    for rr in &raster_regions {
        ...
        let g = gate_one_plate(io, spec, Some(&comp), rr, None);
        ...
        if let Some((w, h)) = g.size {
            ...
            entry.insert("assetHash".into(), json!(g.asset_hash));
            entry.insert("regionHash".into(), json!(g.region_hash));
            entry.insert("referenceHash".into(), json!(g.reference_hash));
            entry.insert("reference".into(), g.reference_audit.unwrap_or(Value::Null));
            entry.insert("compHash".into(), json!(g.comp_hash));
...
fn write_scaffold(io: &Io, spec: &Value) -> Scaffold {
    ...
    for rr in spec_regions(spec) {
        ...
        if medium == "raster" && kind != "texture" {
            ...
            body_parts.push(format!("  <div class=\"r-{id} region control\" data-region=\"{id}\"><!-- {label}: match the control's lettering and visible shape to the crop (impeccable comp-spec --crop {id}); a control need not have button chrome --></div>"));
        } else {
    ...
    css_lines.push("/* Impeccable scaffold: the measured layout of the approved comp as custom properties. Generated by build-phase.mjs scaffold; regenerate after impeccable comp-spec --regions changes. Bind these to your own markup; positions are % of the comp frame so they scale with it. */".into());
...
pub fn run_with_renderer(argv: &[String],io: &mut Io,organic_scan: OrganicScan,renderer: Option<&dyn EntryRenderer>) -> i32 {
    ...
    match cmd {
        ...
        "scaffold" => {
            let Some(spec) = load_spec(&abs(io, SPEC_PATH)) else {
                io.err(&format!("build-phase: no spec at {SPEC_PATH}; run impeccable comp-spec first\n"));
                return 1;
            };
