...
The root harness folders (`.agent/skills/`, `.agents/skills/`, `.claude/skills/`, `.codex/skills/`, `.gemini/skills/`, `.github/skills/`, `.kiro/skills/`, `.opencode/skills/`, `.pi/skills/`, `.vibe/skills/`) and `plugin/` stay tracked so `main` remains installable for direct GitHub, `npx skills`, and submodule users. They are still generated artifacts.
...
## Upstream Rust integration

Read `docs/UPSTREAM-INTEGRATION.md` before changing runtime selection. This fork retains its JavaScript command path, detector and regression suites. `crates/` contains the upstream native engine for explicit evaluation. The skill launcher routes existing sibling commands to Node unless `IMPECCABLE_NATIVE=1`; new native-only verbs and engine probes retain upstream dispatch. Never silently route retrieval to the native remote roll implementation.

For Rust changes, read `docs/ENGINE.md`, add a focused regression, run `cargo test --workspace`, build `cargo build --release -p impeccable`, and run the native oracle against that binary. Do not regenerate frozen `tests/oracle/vectors/calls/` or overwrite goldens to hide a mismatch. Keep `bun run test` and `bun run test:new-work-e2e` passing for the fork command path. Use `cargo xtask bundle` for the native browser/extension bundle and `bun run build:browser` for the fork's JavaScript browser bundle. AI assistance must be disclosed in commits.
