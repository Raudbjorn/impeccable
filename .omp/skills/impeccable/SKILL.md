...
1. Run `<skill-base-dir>/scripts/impeccable context` once per session, where `<skill-base-dir>` is the directory that contains this SKILL.md (the skill folder, not a plugin root two levels above it); keep cwd at the user's project. That base directory resolves every `.omp/skills/impeccable/scripts/impeccable <verb>` command in this skill and its references, and `.omp/skills/impeccable/scripts` is the fallback only when the runtime reports no base directory. The launcher runs a self-contained binary that ships next to it or is downloaded once on first run; the core commands need no Node runtime. Optional helpers need Node.js, including [MiniMax image analysis and web search](reference/minimax.md). Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
...
**Pin / Unpin:** `.omp/skills/impeccable/scripts/impeccable pin <pin|unpin> <command>` creates or removes a standalone `/skill:<command>` shortcut. Report the script's result concisely; relay stderr verbatim on error.

**Hooks:** `/skill:impeccable hooks <on|off|status|ignore-rule|ignore-file|ignore-value|reset>` manages the design detector hook for this project (auto-runs the detector after UI file edits and surfaces findings). Load [hooks.md](reference/hooks.md) when the user invokes it with any argument.

**Doctor:** `/skill:impeccable doctor` reports and repairs drift between this project's Impeccable artifacts (PRODUCT.md, DESIGN.md and its sidecar, config, surface briefs, the hook) and what this version reads. Load [doctor.md](reference/doctor.md) when the user invokes it, or when they ask what is out of date, stale, or needs refreshing. A `CONTEXT_STALE` directive in Setup's output is the cheap subset of the same report; act on it there per its own instructions rather than running doctor unasked.

**Critique-evidence:** `/skill:impeccable critique-evidence [target]` uses catalog citations and detector findings with deterministic scoring. Use only when the user explicitly requests evidence-item or catalog-based scoring; otherwise use standard `critique`. Load [critique-evidence.md](reference/critique-evidence.md).
