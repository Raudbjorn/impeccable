...
```bash
.omp/skills/impeccable/scripts/impeccable live-generate --target src/App.jsx --dev-url http://127.0.0.1:5173/ --selector ".pricing-grid" --action bolder --count 3 --boot --wait-for-browser 60000
```
...
- **`event: null`** with `ok: true`: the event was slower than the wait; run `.omp/skills/impeccable/scripts/impeccable live-poll` once to collect it, then continue.
...
```bash
.omp/skills/impeccable/scripts/impeccable live-poll --reply EVENT_ID done --file src/App.jsx --then-poll
```
...
- **Interrupted or unsure of the state**: `.omp/skills/impeccable/scripts/impeccable live-status`, then `live-resume`; the journal under `.impeccable/live/sessions/` is canonical.
