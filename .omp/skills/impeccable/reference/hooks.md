...
| `reset` | Delete the project config, dedup cache, and pending queue, and remove the hook's entries from every provider manifest `on` installs (a team-shared `.claude/settings.json` that `on` never writes is never touched). |
...
- Do not edit the hook scripts themselves (`hook.mjs`, `hook-lib.mjs`) from this flow. Those are skill plumbing.
- Claude Code, Codex, and GitHub Copilot do not block the edit; they emit a post-edit reminder instead. Disabling stops the reminders.
