...
```
.omp/skills/impeccable/scripts/skill:impeccable doctor --json
```
...
- **`auto`** carries no decision. Run `.omp/skills/impeccable/scripts/skill:impeccable doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
...
- `workspace-platform-native-evidence` is the finding that matters most here: a workspace carrying native build files while inheriting a root record that resolves to web gets web guidance for its whole life and never loads [android.md](android.md). The repair is a child PRODUCT.md in that workspace, because one inherited record cannot hold two platforms.
