...
- A cultural-symbol palette as the obvious pull for a domain that carries one (a nation's flag colors, a cuisine's stock hues, a holiday's palette). Reach past it; let the cultural reading come from typography, imagery, and copy, not the palette. An explicit brief, or an established brand that already pins those colors, overrides this rule.
