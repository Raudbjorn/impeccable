...
is built from this repo's Cargo workspace. Core commands require no Node runtime. The fork's optional design-context import/export, evidence scoring, visual-cue, image-gen, MiniMax image-analysis, and web-search helpers remain separate Node scripts under `skill/scripts/`; their references explicitly invoke `node`.
