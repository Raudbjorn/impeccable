...
- **Validation errors** (stderr, `exitCode 1`): `concept-seed: --scope must be direction or surface`; `--reroll must be a non-negative integer`; `--register must be safer or bolder`; `--register steers a re-roll round; pass --reroll <n> with it`; `--register applies to direction rounds only`; `--mode must be persuade, operate, read, or experience`; `--grain must be one of product, flow, view, region`; `--platform must be one of web, ios, android`; `--candidate-count must be an integer from 5 to 7`; `concept-seed: every challenger tier needs at least one approved concept` (local catalog). Retrieval: `Configure retrieval.command in .impeccable/config.local.json`; `Local retrieval requires --brief-file <task brief> for the first round`; `A session brief is fixed; omit --session to start with a new brief`; `A local retrieval choice requires --session <saved session ID>`; `Cannot read --brief-file <path>: <err>`; and from the transport `Invalid local config: <err>`, `Local retrieval could not start: <err>`, `Local retrieval failed: <detail>`, `Local retrieval timed out; retry the saved session or check catalog doctor --live`, `Retrieval response exceeds 16 MB`. From the response check, one message per field, each surfaced on stderr behind the prefix `Invalid retrieval response: ` (as is a response that is not JSON at all): `unsupported response protocol`, `session must be a non-empty string`, `settings must be an object`, `record must be an object`, `record.challengers must be a non-empty array`, `record.compositions must be a non-empty array`, `session mismatch`, `round mismatch`, `register mismatch`, `settings mismatch: <key>`, `record.challengers has duplicate or missing ids`, `a challenger has no id`, `challenger <id>: system must be an array`, `challenger <id>: unknown well tier <tier>`, `challenger <id>: wellTier must be one of <tiers>`, `record.compositions has duplicate or missing ids`, `a composition has no id`, `composition <id>: grammar must be an array`, `record.staging must be one of record.compositions`, `record.stagings must equal record.compositions`, `choice was not recorded`. The transport errors above and `Invalid session ID` (from `materializeRound`, after the response has already validated) carry no prefix.
