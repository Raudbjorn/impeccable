...
- **Fake mode** `IMPECCABLE_IMAGE_GEN_FAKE=1` (checked after `--score-only`, before the API key): writes the reference crop resized 2x as the plate, tEXt chunks `impeccable:prompt` (the built prompt) and `impeccable:fake=1`; sidecar `${out}.json` = `{prompt, createdAt: ISO, tool: 'impeccable generate-image', model: 'fake', plate: <id>, refs: [<crop path>]}`. stdout `PLATE: ${out} (${w}x${h}, fake 2x crop of region ${id}, $0.00, no API call)`; exit 0; then falls through to scoring.
