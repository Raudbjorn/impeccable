...
- **Setup, before generation or scoring**: loads `--spec` (default `.impeccable/build/spec.json`, `comp_spec::SPEC_PATH`) — missing → stderr `generate-image: no spec at ${spec}; run impeccable comp-spec first` exit 1. Looks up `<id>` in the spec's regions — unknown → stderr `generate-image: no region ${id} in ${spec}; ids: ${comma-joined ids}` exit 1; found but not `medium: raster` → stderr `generate-image: region ${id} is ${medium}, not a plate; set its kind to plate|image|texture in the regions file` exit 1. Reads the comp named in the spec — unreadable → stderr `generate-image: cannot read comp ${comp}: ${err}` exit 1. Writes (or overwrites) the reference crop to `<dir of --spec>/crops/${id}.png`, `impeccable:crop-of` tEXt chunk `${comp}#${id}`, **unconditionally, including under `--score-only`** — a PNG-encode failure here is the only other error shape, stderr `generate-image: ${err}` exit 1.
