...
- **Compose project selection**: `--project-catalog <path>` names a project directory containing `.impeccable/compose`. `--approval-policy human|human-and-machine` defaults to `human`. `--selection-strategy seeded|ranked-1` defaults to `seeded`.
...
- **Args** (each read as `args[idx+1]` after `indexOf`): `--scope` (`surface` default | `direction`), `--from <key>` (default `IMPECCABLE_CONCEPT_SEED` env or `crypto.randomBytes(4).toString('hex')`), `--reroll <n>` (Number, default 0), `--register safer|bolder`, `--mode persuade|operate|read|experience`, `--grain product|flow|view|region`, `--platform web|ios|android`, `--candidate-count 5..7` (default 7), `--chosen <id>`, `--kind assigned|pick|challenger|canon`, `--session <id>` (a retrieval session to continue), `--brief-file <path>` (the task brief, required for a retrieval session's first round), `--replay` (read a saved retrieval round back instead of calling the command). The three retrieval flags require `retrieval.command` in `.impeccable/config.local.json`; passing any of them without it is an error. Flags are validated before the retrieval command runs.
- **Env**: `IMPECCABLE_CONCEPT_SEED`, `IMPECCABLE_CATALOG_DIR` (default = script dir; local catalog files `concept-ingredients.json`, `concept-reviews.json`, `composition-ingredients.json`, `composition-reviews.json`; invalid/missing → treated as absent), `IMPECCABLE_API_URL` (default `https://impeccable.style/api`, trailing `/` stripped), `IMPECCABLE_API_TIMEOUT` (ms, default 4000, one shared deadline for all API calls in the run), `IMPECCABLE_NO_TELEMETRY`, `DO_NOT_TRACK`, `IMPECCABLE_CARD_BASE` (default `https://impeccable.style/worlds/cards`), `IMPECCABLE_COMPOSITIONS=1` (renders composition block; otherwise compositions are dealt but not printed, except for `source:'retrieval'`, whose rounds are validated to carry compositions and always render them).
- **Validation errors** (stderr, `exitCode 1`): `concept-seed: --scope must be direction or surface`; `--reroll must be a non-negative integer`; `--register must be safer or bolder`; `--register steers a re-roll round; pass --reroll <n> with it`; `--register applies to direction rounds only`; `--mode must be persuade, operate, read, or experience`; `--grain must be one of product, flow, view, region`; `--platform must be one of web, ios, android`; `--candidate-count must be an integer from 5 to 7`; `concept-seed: every challenger tier needs at least one approved concept` (local catalog). Retrieval: `Configure retrieval.command in .impeccable/config.local.json`; `Local retrieval requires --brief-file <task brief> for the first round`; `A session brief is fixed; omit --session to start with a new brief`; `A local retrieval choice requires --session <saved session ID>`; `Cannot read --brief-file <path>: <err>`; and from the transport `Invalid local config: <err>`, `Local retrieval could not start: <err>`, `Local retrieval failed: <detail>`, `Local retrieval timed out; retry the saved session or check catalog doctor --live`, `Retrieval response exceeds 16 MB`. From the response check, one message per field, each surfaced on stderr behind the prefix `Invalid retrieval response: ` (as is a response that is not JSON at all): `unsupported response protocol`, `session must be a non-empty string`, `settings must be an object`, `record must be an object`, `record.challengers must be a non-empty array`, `record.compositions must be a non-empty array`, `session mismatch`, `round mismatch`, `register mismatch`, `settings mismatch: <key>`, `record.challengers has duplicate ids`, `a challenger needs a non-empty string id`, `challenger <id>: system must be an array`, `challenger <id>: unknown well tier <tier>`, `challenger <id>: wellTier must be one of <tiers>`, `record.compositions has duplicate ids`, `a composition needs a non-empty string id`, `composition <id>: grammar must be an array`, `record.staging must be one of record.compositions`, `record.stagings must equal record.compositions`, `choice was not recorded`. The transport errors above and `Invalid session ID` (from `materializeRound`, after the response has already validated) carry no prefix.
...
- **Local retrieval** (`retrieval.command` configured): wins outright over the local catalog and the roll API, and a
  failure is an error rather than a fallback, so a broken local command never becomes a remote roll. One JSON request on
  the command's stdin, one JSON response on its stdout; `op` is `start`, `round`, `replay`, or `choose`. Response caps:
  16 MiB stdout, 16 KB of stderr quoted back on a non-zero exit, `timeoutMs` default 180000. Integral numbers cross as
  integers and are compared by value, so `6` and `6.0` agree. Validated before use: protocol version, session and round
  agreement, register agreement, per-key settings agreement, unique challenger and composition ids, known well tiers
  (`graphic`, `interaction`, `atmosphere`), and a staging that is one of the compositions. `materializeRound` requires a
  64-character lowercase-hex session, copies every referenced asset into `.impeccable/retrieval/<session>/` under a
  sha256-named copy, and writes the round to `round-<n>-<safer|bolder|normal>.json`; the register is matched against
  that vocabulary rather than copied from the response, which would otherwise put attacker-chosen text in a path.
  `--chosen` routes to the command's `choose` op, so the remote `/chosen` endpoint is not pinged. Output carries a
  `LOCAL SESSION:` block naming the session and the commands that continue or record against it.
...
- **stdout (full roll)**: header line `${SCOPE} CONCEPT SEED (key: ${key}; mode: ${mode ?? 'unscoped'}; source: ${local|api}; approved pool: ${poolRevision}; ${approvedCount}/${catalogCount} human-approved; rerun with --scope ${scope}[ --mode m] --from ${key}[ --reroll n][ --register r] --candidate-count ${n} to reproduce this roll against this catalog revision)`, then `[RE-ROLL ROUND n ...]`, the assigned block (`ASSIGNED INDEX: N` / `DEALT INDICES: a, b, c (index N leads)` + instruction, or SAFER/BOLDER REGISTER block), `CHALLENGERS:` with `renderChallenger` per concept:
  ...
  ```
  optional composition block, challenger instruction, quality-bar sentence, authority + richness instructions, `A user- or brief-pinned decision beats the roll, always.`, and the restated line (`ASSIGNED INDEX (restated for truncated readers): N. ...; seed key ${key}.`). Degraded: header `... source: degraded; rerun with ...)` then `ASSIGNED INDEX`/`DEALT INDICES`, the "No challengers this run: the roll service was unreachable and no local catalog exists..." paragraph naming the single GET to `https://impeccable.style/api/roll`, authority, pinned-decision line, restated line. Safer register while degraded prints the SAFER block without assignment. Full text templates are in the source (lines 405-690) and should be copied verbatim.
- **Telemetry ping** (`--chosen` and/or `--kind`): disabled unconditionally. No `/chosen` request is sent; stdout is `choice ping skipped\n`, exit 0. Local retrieval feedback remains local.
- **Exit**: destroys undici global dispatcher then `process.exit(exitCode ?? 0)`.
- Tests: `tests/concept-seed.test.mjs` (scopes reproducibility, degraded seed, register, ratings/breadth/mode/grain/platform, init gate, disabled `pingChosen`, API roll path + dispatcher destroy).
...
##### `--plate <region-id>` (engine-only; no `.mjs` predecessor, ported alongside the comp-fidelity verbs below)

Invoked `{{scripts_path}}/impeccable generate-image --plate <id> [--spec <path>] [--out <path>] [--size <WxH>] [--prompt <extra>|--prompt-file <path>] [--chroma|--no-chroma] [--score-only] [--min <n>] [--quality <q>] [--model <model>] [--background transparent|opaque|auto] [--ref <img> repeatable]`. One raster region of a measured comp spec, end to end: the asset-producer's `impeccable comp-spec --crop` + generation + `impeccable comp-spec --plate-prompt` + score sequence in one command.

- **Setup, before generation or scoring**: loads `--spec` (default `.impeccable/build/spec.json`, `comp_spec::SPEC_PATH`) — missing → stderr `generate-image: no spec at ${spec}; run impeccable comp-spec first` exit 1. Looks up `<id>` in the spec's regions — unknown → stderr `generate-image: no region ${id} in ${spec}; ids: ${comma-joined ids}` exit 1; found but not `medium: raster` → stderr `generate-image: region ${id} is ${medium}, not a plate; set its kind to plate|image|texture in the regions file` exit 1. Reads the comp named in the spec — unreadable → stderr `generate-image: cannot read comp ${comp}: ${err}` exit 1. Writes (or overwrites) the reference crop to `<dir of --spec>/crops/${id}.png`, `impeccable:crop-of` tEXt chunk `${comp}#${id}`, **unconditionally, including under `--score-only`** — a PNG-encode failure here is the only other error shape, stderr `generate-image: ${err}` exit 1. `--min`, when present, is parsed up front and must be a finite number (`1.1` is legal; the gate is a comparison, not a percentage bound) — anything else is stderr `generate-image: --min ${value} is not a finite number` exit 1, before the crop write and before any API call. Unlike every other option here, a present `--min` whose value is empty, missing, or another flag is that same error rather than an absent option: a threshold asked for and silently not applied is the one failure this option must not have.
- **Output path**: `--out`, else the region's own `plate` field from the spec, else stderr `generate-image: region ${id} has no "plate" path in ${spec}; re-run comp-spec --regions or pass --out <path>` exit 1 (an explicit empty-string `plate` in the regions file counts as absent here, since `comp-spec` preserves it verbatim rather than defaulting it).
- **Size**: `--size`, else picked from the region's `px.w`/`px.h` aspect so the plate clears the plates gate's >= 1.5x-width floor (capped at 1536): landscape (aspect > 1.2) or a portrait/square region needing more than 1024 width → `1536x1024` (portrait, aspect < 0.83, needing <= 1024 width → `1024x1536`); everything else → `1024x1024`.
- **Prompt**: `comp_spec::plate_prompt(spec, region)`, plus `--prompt`/`--prompt-file` appended verbatim, plus (when chroma applies) one fixed sentence describing the flat green key background — the three parts space-joined, empty parts dropped.
- **Model and background**: defaults to `gpt-image-2.5-flare`; `--model` overrides it. `--background` requests native PNG output and records `background` and `outputFormat` in the sidecar. Transparent backgrounds use the cutout prompt and preserve the returned alpha.
- **Chroma**: an explicit `--background` disables keying, including when `--chroma` is present. Otherwise `--chroma`/`--no-chroma` force it; otherwise on by default when the region's `kind` is `plate` and its sampled palette reads as ink over one dominant ground color (`ink_on_ground`) — an illustration or figure on flat ground generates on `#00ff00` and gets keyed to alpha after generation, so the page's own ground shows through instead of a second paper.
- **`--score-only`**: skips generation and the API-key check entirely (the native-tool branch is exactly the no-key case this exists for). The plate named by `--out`/the spec must already exist on disk — missing → stderr `generate-image: no plate at ${out} to score; produce it first, then run --score-only` exit 1. Otherwise scores it directly (see Scoring below).
- **Fake mode** `IMPECCABLE_IMAGE_GEN_FAKE=1` (checked after `--score-only`, before the API key): writes the reference crop resized 2x as the plate, tEXt chunks `impeccable:prompt` (the built prompt) and `impeccable:fake=1`; sidecar `${out}.json` = `{prompt, createdAt: ISO, tool: 'impeccable generate-image', model: 'fake', plate: <id>, refs: [<crop path>]}`. stdout `PLATE: ${out} (${w}x${h}, fake 2x crop of region ${id}, $0.00, no API call)`, then falls through to scoring, which decides the exit code. A write failure to `--out` takes the same shape as the real path's: stderr `Error: ${node-style message}` exit 1. The sidecar write stays best-effort.
- **Real mode**: no `OPENAI_API_KEY` → stderr `generate-image: OPENAI_API_KEY is not set; use the harness-native image tool instead.` exit 1. `--quality` defaults to `high` (not the base mode's `medium`). Refs sent to the API are the reference crop plus any `--ref`. Same OpenAI request/error shapes as the base mode above (`call_openai_image`); a write failure to `--out` → stderr `Error: ${node-style message}` exit 1. Embeds the prompt (best-effort, same failure message as the base mode) and writes sidecar `${out}.json` = `{prompt, createdAt, tool: 'impeccable generate-image', model: <selected model>, refs}`; stdout `IMAGE: ${out} (${size}, ${quality}, ${model}, billed to your OpenAI key); ${prompt embedded + sidecar|sidecar} at ${out}.json`. When chroma applies, keys the plate in place and prints `PLATE-CHROMA keyed ${pct}% of pixels to alpha (#00ff00); place with a plain <img> over the page's own ground, no background on the plate.` (plus a regenerate-with-`--no-chroma` hint appended when the keyed fraction is under 20%, meaning the generator ignored the key) — a keying failure prints to stderr instead (`generate-image: chroma key failed: ${err}`) and does not stop the run. Then scores (see below).
- **Scoring** (`--score-only` and the tail of every generate path): runs the same gate `impeccable build-phase advance` scores plates with (`build_phase::gate_one_plate`, scoring the file at `--out`/the spec's `plate` path against the region). A score prints `PLATE-SCORE ${id} ${overall}% against the comp region (structure ${s}%, color ${c}%, detail ${d}%)` (0 decimals); every gate reason prints its own `PLATE-WARN ${reason}` line. No usable score → stderr `generate-image: no score for ${out}; the plates gate refuses it as it stands.` exit 1. With `--min <n>` and the score under it → `PLATE-REJECTED below --min ${n}%` exit 3 (checked before the gate-reasons exit below). Any gate reason still present → stderr `generate-image: ${out} does not pass the plates gate; open it beside ${crop} and regenerate before building on it.` exit 2. Clean (a score, no reasons, at/above `--min`) → exit 0.
- Tests: `crates/context/src/generate_image.rs`'s `plate_tests` module (unit, fake mode; unknown region, non-raster region, empty explicit `plate` path, missing on-disk plate under `--score-only`, `--min` rejection all covered) and `tests/oracle/cases/comp.mjs`'s `genimg-plate-*` cases (fake generation, chained score-only against the same plate including the no-key/no-fake-flag step, `--min` rejection, missing-plate/unknown-region/non-raster-region/missing-spec/empty-plate-path errors), replayed against `comp-basic`'s pre-measured spec.
