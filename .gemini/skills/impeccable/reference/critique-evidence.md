...
Send the LLM the target page (HTML and/or screenshot) plus the full evidence catalog: the 10 files under `.gemini/skills/impeccable/scripts/data/critique-evidence/heuristic-*.json`, one per Nielsen heuristic. The LLM returns a JSON list of `{heuristic_id, item_id, citation}` triples drawn ONLY from the catalog. No score, no prose summary.
...
Run:
```bash
node .gemini/skills/impeccable/scripts/score-evidence.mjs <merged-items.json>
```
