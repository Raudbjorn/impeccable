...
// The tab that adopts this session was loaded from the picker server's port,
// not this one, so its fetches carry that origin; that is the one origin
// that ever legitimately calls here. picker-server.mjs passes its own port
// as --origin-port when it forks this process.
const originPort = Number(readArg('--origin-port', '0'));
const allowedOriginHeader = originPort ? `http://127.0.0.1:${originPort}` : '';
if (!port || !token) {
