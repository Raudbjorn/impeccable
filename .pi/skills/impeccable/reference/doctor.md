...
```
.pi/skills/impeccable/scripts/impeccable doctor --json
```
...
- **`auto`** carries no decision. Run `.pi/skills/impeccable/scripts/impeccable doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
